...
export function AdminPage() {
  ...
  const handleToggleComp = async (row: AdminUserRow) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Could not update comp access.')
    } finally {
    ...
    <div className="flex flex-col gap-6">
      <div>
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400">
          Manage users and grant complimentary (free) access for testing — up to {COMP_ACCESS_LIMIT} accounts at a
          time.
        </p>
      </div>
      ...
      {loading ? (
        ...
        <EmptyState title="No users yet" description="Once people sign up, they'll show up here." />
      ) : (
        <div className="overflow-hidden rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900">
          <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-700 px-5 py-3">
            ...
            <span className="text-xs text-slate-500 dark:text-slate-400">
              Comp access: {compCount} / {COMP_ACCESS_LIMIT}
            </span>
          </div>
            ...
            <table className="w-full text-left text-sm">
              <thead>
                <tr className="border-b border-slate-200 dark:border-slate-700 text-xs uppercase tracking-wide text-slate-500 dark:text-slate-400">
                  ...
                  <th className="px-5 py-2 font-medium">Comp access</th>
                </tr>
              </thead>
              <tbody>
                {rows.map((row) => {
                  ...
                  return (
                    <tr key={row.user_id} className="border-b border-slate-100 dark:border-slate-800 last:border-0">
                      ...
                      <td className="px-5 py-3 text-slate-700 dark:text-slate-300">
                        {TIERS[row.subscription_tier]?.label ?? row.subscription_tier}
                      </td>
                      <td className="px-5 py-3 text-slate-500 dark:text-slate-400">{row.subscription_status}</td>
                      ...
                      <td className="px-5 py-3">
                        <button
                          ...
                          title={
                            isSelf
                              ? "You can't change your own comp access"
                              : !row.is_comp_access && !canGrant
                                ? 'Comp access limit reached'
                                : undefined
                          }
                        ...
                        >
                          {pendingUserId === row.user_id ? 'Saving…' : row.is_comp_access ? 'Granted' : 'Grant'}
                        </button>
                      </td>
                    </tr>
                  )
