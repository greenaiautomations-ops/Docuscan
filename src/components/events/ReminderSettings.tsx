...
export function ReminderSettings() {
  ...
  if (loading) return <p className="text-sm text-slate-500 dark:text-slate-400">Loading reminder settings…</p>
  if (!prefs) return <p className="text-sm text-slate-500 dark:text-slate-400">{error ?? 'Reminder settings unavailable.'}</p>
        ...
        {OPTIONS.map((opt) => (
          <label key={opt.key} className="flex cursor-pointer items-center justify-between gap-4 px-4 py-3">
            <div>
              ...
              <p className="text-xs text-slate-400 dark:text-slate-500">{opt.hint}</p>
            </div>
            <input
              type="checkbox"
