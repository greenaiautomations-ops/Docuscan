...
export function ExtractedDataRaw({ data }: { data: ExtractedData }) {
  ...
  return (
    <div className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-5">
      ...
      >
        <h2 className="text-sm font-semibold text-slate-700 dark:text-slate-300">Extracted Data</h2>
        <span className="text-xs text-slate-400 dark:text-slate-500">{open ? 'Hide raw JSON' : 'Show raw JSON'}</span>
      </button>
      {open && (
